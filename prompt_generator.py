import json
prompt_template = f"""
你是一个高度智能、注重细节的中文校对批处理引擎。你的任务是接收一个JSON数组，并对每个任务进行独立的、多维度的校对分析。
...
prompt_template_2 = f"""
你是一个高度智能、注重细节的中文校对批处理引擎。你的任务是接收一个JSON数组，并对每个任务进行独立的、基于修正说明的校对。
...
json.dump(
    prompt_template,
    open("prompt_template.txt", "w", encoding="utf-8"),
    ensure_ascii=False,
    indent=4
)
...
json.dump(
    prompt_template_2,
    open("prompt_template_2.txt", "w", encoding="utf-8"),
    ensure_ascii=False,
    indent=4
)